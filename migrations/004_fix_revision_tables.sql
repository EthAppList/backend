-- Fix Revision Tables Migration
-- This migration fixes the ID generation issues in revision tables for Railway deployment

-- First, check if the tables exist and drop them if they do
-- We'll recreate them with proper ID generation that works universally
...
-- Ensure products table has the revision fields (in case they were missing)
...
UPDATE products SET current_revision_number = 1 WHERE current_revision_number IS NULL; 